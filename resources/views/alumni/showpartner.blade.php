...
<div class="w3-container w3-content" style="max-width:1400px;margin-top:80px">    
  ...
  <div class="w3-row">
    ...
    <div class="w3-col m7">
      ...
      @endif
        <h1>Company Name: {{ $partner->title }}</h1>
     <h2>Description: {{ $partner->description }}</h2>
    <p class="lead">{{ $partner->contact }}</p>
    <p class="lead">{{ $partner->email }}</p>
    <p class="lead"> {{ $partner->location }}</p>
    <p class="lead">{{ $partner->branches }}</p>
 ...
 <div class="w3-col m2">
      <div class="w3-card w3-round w3-white w3-center">
        ...
        <p>{{ date('M j, Y h:ia', strtotime($partner->created_at)) }}</p><hr>
     <label>Last Updated:</label>
     <p>{{ date('M j, Y h:ia', strtotime($partner->updated_at)) }}</p>
